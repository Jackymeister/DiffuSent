...
class SpanAsteModel(nn.Module):
    ...
    def __init__(
        ...
        self.bert = BertModel.from_pretrained("./Pytorch-Span-ASTE/bert-large-cased")
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, seq_len):
        ...
        x = bert_output[0]
        spans, span_indices = self.span_representation(x, batch_max_seq_len) #[1,84,2068] [84,2]

        spans_probability = self.span_ffnn(spans) #[1,84,3]->分类a/o/none
        nz = int(batch_max_seq_len * self.span_pruned_threshold)

        target_indices, opinion_indices = self.pruned_target_opinion(spans_probability, nz) #7,7 在span_indices中索引
        ...
        candidates, candidate_indices, relation_indices = self.target_opinion_pair_representation(
            spans, span_indices, target_indices, opinion_indices)
        #[1,49,4264], [1,49,4] (a,b,c,d)->offset, [1,49,2] (span1,span2)->在span_indices中的索引
        candidate_probability = self.pairs_ffnn(candidates) #[1,49,4] -> 分类n/o/p/none

        # batch span indices
        span_indices = [span_indices for _ in range(batch_size)] #[1,84,2] 所有span的offset

        return spans_probability, span_indices, candidate_probability, candidate_indices
        #    [1,84,3]->分类a/o/none| [1,84,2] 所有span的offset| [1,49,4] -> 分类n/o/p/none| [1,49,4] (a,b,c,d)->offset
