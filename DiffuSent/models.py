...
class DiffusionNER(PreTrainedModel):
    ...
    def prepare_diffusion_concat(self, gt_spans, gt_num):
        ...
        if not num_gt:  # generate fake gt boxes if empty gt boxes
            ...
            num_gt = 1
        # print(num_gt)
        print((num_gt,self.num_proposals))
    ...
    def backbone(self, 
        ...
        h_token_lstm = None
        if self.lstm_layers > 0:
            print(h_token.shape) # torch.Size([16, 128, 1024])
            token_count = token_masks.long().sum(-1,keepdim=True)
        ...
        print(h_token_lstm.shape) # torch.Size([16, #, 1024])
    ...
    def left_right_type(self, h_token, h_token_lstm, span_mask, timestep_embeddings, span, token_count, token_masks, type_embeddings, span_ct, is_train):
        ...
        # np.save('DiffusionIE_11.21/data/case/numpy_data.npy', numpy_data)
