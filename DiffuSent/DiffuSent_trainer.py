...
sys.path.append(os.path.abspath('./DiffusionIE_11.21/utils'))
sys.path.append(os.path.abspath('./DiffusionIE_11.21/models'))
...
class DiffusionNERTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        relation_type=4
        compute_loss = DiffusionNERLoss(relation_type, self._device, model, optimizer, scheduler, args.max_grad_norm, args.nil_weight, args.match_class_weight, args.match_boundary_weight, args.loss_class_weight, args.loss_boundary_weight, args.match_boundary_type, args.type_loss, solver = args.match_solver)
        # train
        best_f1 = 0
        test_f1 = 0
        ...
        for epoch in range(args.epochs):
            ...
            # t_f1 = self._eval(model, test_dataset, test_dataloader, epoch + 1, updates_epoch)
    ...
    def _train_epoch(self, model: torch.nn.Module, optimizer: Optimizer, compute_loss: Loss, scheduler, train_dataloader, 
        ...
        for batch_ix, batch in enumerate(train_dataloader):
            ...
            print("context_masks",attention_mask.shape)
            print("seg_encoding",attention_mask.shape)
            print("context2token_masks",type(context2token_masks))
            print("token_masks",token_masks.shape)

            print("entity_spans",gt_rel1.shape)
            print("entity_types",gt_rel_labels1.shape)
            print("entity_masks",gt_masks1.shape)

            '''
            encodings torch.Size([16, 128])
            context_masks torch.Size([16, 128])
            seg_encoding torch.Size([16, 128])
            context2token_masks ([], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [])
            token_masks torch.Size([16, 128])
            
            entity_spans torch.Size([16, 60, 4])
            entity_types torch.Size([16, 60])
            entity_masks torch.Size([16, 60])
            torch.Size([16, 55, 1024])
            '''
    ...
    def _eval(self, model: torch.nn.Module, eval_dataset, eval_dataloader,
        ...
        with torch.no_grad():
            ...
            for batch_ix, batch in enumerate(eval_dataloader):
                input_ids, attention_mask,  spans, relations, span_labels, relation_labels, seq_len,context2token_masks,\
                 token_masks, gt_spans, gt_types, gt_masks, raw_len, gt_rel, gt_rel_lables,word2span = batch
                input_ids = torch.tensor(input_ids, device=self._device)
                attention_mask = torch.tensor(attention_mask, device=self._device)
                #token_type_ids = torch.tensor(token_type_ids, device=self._device)
                max_len=max(raw_len)
                ...
                #     pad_mask=pad_mask+[[False]*args.max_seq_len]*pad_len
